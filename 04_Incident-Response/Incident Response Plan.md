...
This plan is designed to handle all cybersecurity issues that could impact FinCorp’s information systems, data, and employees. Considering an incident as any event that could harm the privacy, accuracy, or accessibility of an information system or the information it manages.
...
*Before an incident occurs, ensure that the necessary capabilities are in place to handle it effectively by getting the appropriate tools and skills.*

*Preparation is not a reaction to an incident it is the continuous effort that enables an effective response*
...
**Critical Rule:** *Take a forensic image or snapshot BEFORE you clean anything.* Evidence preservation is legally and operationally essential. Once you eradicate the evidence, it may be gone.
...
1. **Restore systems** per the chosen method
2. **Validate functionality** confirm services are running, data is intact, users can access what they need
3. **Monitor closely** enhanced monitoring for minimum 48 hours post-recovery to detect recurrence
4. **Declare recovery complete** when no anomalous activity is detected in the monitoring period
